...
import EditClanModal from '@/components/mentees/EditClanModal'
...
export default function MenteesPage() {
    ...
    function openEditModal(mentee) {
        ...
        setNewClan(mentee.clanName || '')
        setEditModalOpen(true)
    }

    async function saveClanChange() {
        try {
            const res = await authFetch(`${process.env.NEXT_PUBLIC_API_URL}/api/mentees/${editingMentee._id}`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ clanName: newClan })
            })
    ...
    return (
        <div className="space-y-6">
            ...
            <EditClanModal
                ...
                onSave={saveClanChange}
                clans={clans}
                currentClan={newClan}
                onClanChange={setNewClan}
            />
